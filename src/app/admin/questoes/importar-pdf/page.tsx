...
import { renderPdfToPages, type PageImage } from "@/lib/pdf"
import {
  QuestaoForm,
  emptyDraft,
  validarDraft,
...
type Fase = "idle" | "processando" | "revisao"
...
export default function ImportarPdfPage() {
  ...
  async function handleExtrair() {
    if (!file) { toast.error("Selecione um PDF primeiro"); return }
    if (file.type !== "application/pdf") { toast.error("O arquivo precisa ser um PDF"); return }

    setFase("processando")
    setCards([])
    setProgresso({ atual: 0, total: 0 })

    // 1. Renderiza as páginas no navegador.
    ...
    for (let i = 0; i < renderizadas.length; i++) {
      ...
      setStatusTxt(`Lendo página ${page.pageNumber} de ${renderizadas.length} com IA...`)
      try {
        ...
        if (!res.ok) {
          ...
          const novos = (json.questoes as QuestaoExtraida[]).map((q) => extraidaParaCard(q, page.pageNumber))
          if (novos.length > 0) {
      ...
      setProgresso({ atual: page.pageNumber, total: renderizadas.length })
    }
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl mx-auto space-y-6">
      ...
      {fase === "revisao" && (
        <div className="flex items-center justify-between bg-card border border-card-border rounded-card p-4">
          ...
          <button onClick={() => { setFase("idle"); setCards([]); setFile(null) }} className="text-xs text-muted hover:text-foreground font-mono">
            ↻ OUTRO PDF
          </button>
        </div>
      )}
